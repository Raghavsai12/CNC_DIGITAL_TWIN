...
// Define the Schema for IoT Time-Series Data (Updated for Laser Twin)
...
accel_x: Number,
accel_y: Number,
air_gas: Number, // 0.5 to 1.0 range
...
app.get('/', (req, res) => {
res.send('CNC Laser Digital Twin Backend is LIVE!');
});

// MQTT CLIENT & DATA PRODUCER
...
if (IS_MOCK_MODE) {
console.log('(SYSTEM) MOCK MODE is ON. Generating fake CNC Laser telemetry...');
...
const mockData = {
serial_no: "MOCK-LASER-100",
status: isAnomaly ? "ERROR" : "RUNNING",
temp: parseFloat((30.0 + Math.random() * 15.0).toFixed(2)),
// Acceleration logic (-1.0 to 1.0 G normal, spike up to 2.5 on anomaly)
accel_x: isAnomaly ? parseFloat((2.0 + Math.random() * 0.8).toFixed(3)) : parseFloat((-1.0 + Math.random() * 2.0).toFixed(3)),
accel_y: parseFloat((-1.0 + Math.random() * 2.0).toFixed(3)),
// Air Gas logic (0.5 to 1.0 normal, drops if anomaly)
air_gas: isAnomaly ? parseFloat((0.1 + Math.random() * 0.2).toFixed(2)) : parseFloat((0.5 + Math.random() * 0.5).toFixed(2)),
timestamp: Date.now(),
isMock: true
};
