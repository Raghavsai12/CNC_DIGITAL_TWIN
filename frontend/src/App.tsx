...
import { Activity, CheckCircle, AlertTriangle, ShieldAlert, Cpu, Clock, Target } from 'lucide-react';
import { io } from 'socket.io-client';

const BACKEND_URL = import.meta.env.VITE_BACKEND_URL || 'http://localhost:5000';
const socket = io(BACKEND_URL);

// UPDATED INTERFACES FOR LASER SYSTEM
interface TelemetryData { time: string; temp: number; accelX: number; accelY: number; airGas: number; }
...
const [data, setData] = useState<TelemetryData[]>([{ time: '00:00', temp: 0, accelX: 0, accelY: 0, airGas: 0 }]);
...
const [currentAccel, setCurrentAccel] = useState({ x: 0, y: 0 });
const [currentGas, setCurrentGas] = useState(0); // NEW: Air Gas State
const [isAnomaly, setIsAnomaly] = useState(false);

const [oeeScore, setOeeScore] = useState(98.5);
const [toolHealth, setToolHealth] = useState(100.0); // Will be displayed as Laser Source Health
...
temp: payload.temp || 0,
accelX: payload.accel_x || 0,
accelY: payload.accel_y || 0,
airGas: payload.air_gas || 0
};

setCurrentTemp(payload.temp || 0);
setCurrentAccel({ x: payload.accel_x || 0, y: payload.accel_y || 0 });
setCurrentGas(payload.air_gas || 0);
setStatus(payload.status || 'UNKNOWN');

const currentlyAnomaly = payload.is_anomaly === true;
setIsAnomaly(currentlyAnomaly);
// CALCULATE LASER SOURCE DEGRADATION
const maxAccel = Math.max(Math.abs(payload.accel_x || 0), Math.abs(payload.accel_y || 0));
const gasLevel = payload.air_gas || 1.0;

// Degrade health faster if acceleration is violent OR if air gas is dangerously low
if (maxAccel > 1.8 || gasLevel < 0.6) {
setToolHealth(prev => Math.max(0, prev - (maxAccel * 0.05) - (gasLevel < 0.6 ? 0.1 : 0)));
}
...
setTotalAnomalyCount(prev => prev + 1);
...
reason: payload.reason || 'AI Kinematic Match'
};
...
const getAccelColor = (val: number) => Math.abs(val) > 1.8 ? 'text-red-400 font-bold' : 'text-emerald-400';
const getTempColor = (val: number) => val > 45 ? 'text-red-400 font-bold' : 'text-emerald-400';
const getGasColor = (val: number) => val < 0.6 || val > 1.0 ? 'text-amber-400 font-bold' : 'text-emerald-400';
return (
...
<div>
<h1 className='text-3xl font-black text-white tracking-tight'>LASER COMMAND CENTER</h1>
...
{/* CARD 2: LASER SOURCE HEALTH */}
<div className='bg-[#111827] border border-slate-800 p-6 rounded-2xl relative overflow-hidden flex flex-col justify-center'>
<h3 className='text-slate-400 text-sm font-semibold tracking-wider uppercase mb-6 flex items-center gap-2'>
<Target size={16} className='text-blue-500' /> Laser Source Health
</h3>
...
{toolHealth < 30 ? 'MAINTENANCE REQUIRED' : 'Nominal Power'}
</span>
...
<div className='bg-[#111827] border border-slate-800 p-6 rounded-2xl relative overflow-hidden flex flex-col justify-center'>
<h3 className='text-slate-400 text-sm font-semibold tracking-wider uppercase mb-6 flex items-center gap-2'>
<Clock size={16} className='text-emerald-500' /> Shift Uptime
</h3>
...
<div>
<h3 className='text-slate-300 font-bold flex items-center gap-2 mb-2'>
<Activity className='text-blue-500' size={18} /> Real-Time Head Kinematics & Gas
</h3>
...
<span>TEMP &lt; 45°C</span>
<span>ACCEL(X,Y) &lt; 1.8G</span>
<span>GAS &gt; 0.6 MPa</span>
</div>
</div>
...
<span className='text-slate-500 text-[10px] mb-1'>LIVE TEMP</span>
<span className={getTempColor(currentTemp)}>{currentTemp.toFixed(1)}°C</span>
</div>
<div className='flex flex-col items-center'>
<span className='text-slate-500 text-[10px] mb-1'>X-ACCEL</span>
<span className={getAccelColor(Math.abs(currentAccel.x))}>{currentAccel.x.toFixed(2)}G</span>
</div>
<div className='flex flex-col items-center'>
<span className='text-slate-500 text-[10px] mb-1'>Y-ACCEL</span>
<span className={getAccelColor(Math.abs(currentAccel.y))}>{currentAccel.y.toFixed(2)}G</span>
</div>
<div className='flex flex-col items-center'>
<span className='text-slate-500 text-[10px] mb-1'>AIR GAS</span>
<span className={getGasColor(currentGas)}>{currentGas.toFixed(2)} MPa</span>
</div>
</div>
</div>
...
<YAxis yAxisId="left" stroke='#4b5563' tick={{ fill: '#6b7280', fontSize: 12 }} domain={['auto', 'auto']} />
{/* Added a right Y-axis specifically for the Air Gas so it scales nicely (0 to 1.5) */}
<YAxis yAxisId="right" orientation="right" stroke='#f59e0b' tick={{ fill: '#f59e0b', fontSize: 12 }} domain={[0, 1.5]} />

<Tooltip contentStyle={{ backgroundColor: '#0f172a', border: '1px solid #1e293b' }} />
<Legend wrapperStyle={{ paddingTop: '10px' }} iconType='circle' />

{/* Acceleration Lines */}
<Line yAxisId="left" type='monotone' name='X-Axis Accel' dataKey='accelX' stroke='#3b82f6' strokeWidth={2} dot={false} isAnimationActive={false} />
<Line yAxisId="left" type='monotone' name='Y-Axis Accel' dataKey='accelY' stroke='#10b981' strokeWidth={2} dot={false} isAnimationActive={false} />

{/* Air Gas Line mapped to the right axis */}
<Line yAxisId="right" type='monotone' name='Air Gas (Bar)' dataKey='airGas' stroke='#f59e0b' strokeWidth={2} dot={false} isAnimationActive={false} />

{isAnomaly && <Line yAxisId="left" type='step' dataKey='accelX' stroke='#ef4444' strokeWidth={4} dot={false} isAnimationActive={false} />}
</LineChart>
</ResponsiveContainer>
</div>
</div>
</div>
{/* SIDEBAR: AI SECURITY LOG */}
...
<p className='text-sm font-medium'>System operating within normal parameters.</p>
</div>
) : (
...
<span className='text-xs font-bold text-red-400 bg-red-500/10 px-2 py-0.5 rounded'>FAULT</span>
</div>
